function Create(self)
	...
	self.attachable:SetStringValue("Belt Name", self.PresetName);
	...
	self.attachable:SetNumberValue("Belt Mass", self.beltMass);
...
function Update(self)		--For some reason onAttach causes issues
	...
	if self.rootParent and self:IsAttached() then
		if self.addedBelt == false and (not self.rootParent:NumberValueExists(self.PresetName) or (self.rootParent:NumberValueExists(self.PresetName) and self.rootParent:GetNumberValue(self.PresetName) <= 0)) then
