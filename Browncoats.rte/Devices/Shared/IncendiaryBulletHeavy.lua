function Create(self)
	...
	trail.Pos = self.Pos - Vector(self.Vel.X, self.Vel.Y) * rte.PxTravelledPerFrame/2;
...
function Update(self)
	local velFactor = math.floor(1 + math.sqrt(self.Vel.Magnitude)/(1 + self.Age/100));
	...
	trail.Pos = self.Pos - Vector(self.Vel.X, self.Vel.Y):SetMagnitude(math.min(self.Vel.Magnitude * rte.PxTravelledPerFrame, self.trailLength)/2);
	trail.EffectRotAngle = self.Vel.AbsRadAngle;
	MovableMan:AddParticle(trail);
