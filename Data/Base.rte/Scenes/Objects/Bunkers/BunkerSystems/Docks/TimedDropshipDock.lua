...
function Update(self)
	...
	if self.HoldTimer:IsPastSimMS(self.HoldTime) then --This block runs after it's passed
		if self.HasDockedCraft then
			self.craft.AIMode = Actor.AIMODE_RETURN;
			self.HasDockedCraft = false;
		end
	end
	if self.craft and IsActor(self.craft) then
		...
		if self.HasDockedCraft then
			if self.craft.AIMode == Actor.AIMODE_STAY then
	...
	elseif self.updateTimer:IsPastSimMS(200) then
		...
		for actor in MovableMan.Actors do
			--See if a live dropship is within 50 pixel range of the docking unit
			if (actor.ClassName == "ACDropShip") and actor.Health > 0 and (math.abs(actor.Pos.X - self.Pos.X) < self.detectionRange) and (math.abs(actor.Pos.Y - self.Pos.Y) < self.detectionRange) then
