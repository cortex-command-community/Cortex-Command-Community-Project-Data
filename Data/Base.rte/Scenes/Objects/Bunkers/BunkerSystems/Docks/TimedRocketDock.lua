...
function Update(self)
	...
	elseif self.updateTimer:IsPastSimMS(200) then
		...
		for actor in MovableMan.Actors do
			...
			if (actor.ClassName == "ACRocket") and actor.Health > 0 and (math.abs(actor.Pos.X - self.Pos.X) < self.detectionRange) and (math.abs(actor.Pos.Y - self.Pos.Y) < self.detectionRange) then
