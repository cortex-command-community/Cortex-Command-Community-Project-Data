...
function Update(self)
	if self.switched == false then
		...
		if actor.Sharpness ~= 0 then
			if actor.Sharpness == 3 then
				for i = 1,MovableMan:GetMOIDCount()-1 do
					...
					if part and part.RootID == self.RootID and part.PresetName == "Ronin Heavy Arm FG" then
						self.partWatch = part;
						break;
					end
				end
		...
		if self.partWatch ~= nil and self.partWatch.ID ~= 255 and self.partWatch.PresetName == "Ronin Heavy Arm FG" then
